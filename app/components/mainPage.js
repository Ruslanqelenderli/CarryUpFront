...
function MainPage() {
  ...
  const [totalTripsData, setTotalTripsData] = useState(null);
  const [loading, setLoading] = useState(false);

  const lastIndex = currentTripPage * tripPerPage;
  ...
  const getTrips = async () => {
    try {
      ...
      if (responseData?.list.length > 0) {
        ...
        setTotalTripsData(responseData.totalCount);
      } else {
        setTripData(responseData.list);
  ...
  const nextPage = () => {
    ...
    console.log("currentTripPage", currentTripPage);
  };
  ...
  const prePage = () => {
    ...
    console.log("currentTrkkipPage", currentTripPage);
  };

  function changeCPage(id) {
    setCurrentTripPage(id);
  }
  ...
  return (
    <div className={style.profile}>
      ...
      <div className="flex justify-between mx-10 ">
        ...
        <div className="bg-white aside shadow-md   relative  md:w-[77%] lg:w-[80%] xl:w-[80%%] col-span-2  mt-4 mr-4 relative ">
          {loading && (
          ...
          {activeButton === "forCarry" && (
            <>
              <div className="grid grid-cols-3 gap-4 py-6 px-6 ">
                {tripData.map((v) => (
                  <>
                    <div className="bg-white border box relative pt-4 border-[#A0CCFF] border-solid rounded-[1rem]  h-52 min-h-56 hover:bg-[#449aff29] hover:border-[#0E6FE1] hover:shadow-xl transition duration-700 ease-in-out">
                      ...
                      <div className="flex">
                        <Tooltip text={v.description}>
                          <p className="px-4 mt-3 material-symbols-outlined ">
              ...
              <CustomPagination
                ...
                nextPage={nextPage}
                prePage={prePage}
                totalTrips={totalTripsData}
                tripPerPage={tripPerPage}
                setCurrentPage={setCurrentTripPage}
